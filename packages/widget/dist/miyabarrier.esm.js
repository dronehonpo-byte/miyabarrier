...
`);case"site":return t.site;case"path":return t.path;case"date":return t.at.slice(0,10);default:return""}}),qe=(e,t)=>({to:t.email.trim(),subject:je(e.subject,t).replace(/\s+/g," ").trim(),body:je(e.body,t),context:t}),ye=()=>{try{let e=localStorage.getItem(fe),t=e?JSON.parse(e):[];return Array.isArray(t)?t:[]}catch{return[]}},Ge=(e,t)=>{try{let n=ye();if(n.some(o=>o.to.toLowerCase()===e.to.toLowerCase()))return"duplicate";let r=[...n,e].slice(-Math.max(1,t));return localStorage.setItem(fe,JSON.stringify(r)),"queued"}catch{return"failed"}};var Ve=()=>{try{localStorage.removeItem(fe)}catch{}};var zt={strokeWidth:.8,rings:14,offset:16,radius:33,innerRadius:12,flatten:.64,tilt:116,drift:11},Je={rings:5,strokeWidth:2.2,innerRadius:16,drift:6},Kt=(e,t)=>{let n=[],r=e*Math.PI/180;for(let o=0;o<t.rings;o+=1){let a=t.rings===1?1:o/(t.rings-1),i=t.innerRadius+(t.radius-t.innerRadius)*a,l=i*t.flatten,u=t.offset+(1-a)*t.drift,d=Math.cos(r)*u,s=Math.sin(r)*u,m=e+t.tilt,h=(.45+.55*(1-a*.55)).toFixed(3);n.push(`<ellipse cx="${d.toFixed(2)}" cy="${s.toFixed(2)}" rx="${i.toFixed(2)}" ry="${l.toFixed(2)}" transform="rotate(${m.toFixed(1)} ${d.toFixed(2)} ${s.toFixed(2)})" opacity="${h}"/>`)}return n.join("")},ie=(e={})=>{var l;let t={...zt,...e},n=(l=e.idPrefix)!=null?l:"mb-logo",r=e.monochrome?"currentColor":`url(#${n}-grad)`,o=e.monochrome?"currentColor":`url(#${n}-core)`,a=Array.from({length:3},(u,d)=>Kt(d*(360/3)-96,t)).join("");return`<svg viewBox="-50 -50 100 100" xmlns="http://www.w3.org/2000/svg" fill="none" aria-hidden="true">${e.monochrome?"":`<defs>
...
</radialGradient></defs>`}<g stroke="${r}" stroke-width="${t.strokeWidth}">${a}</g><ellipse cx="-2" cy="1" rx="12" ry="10" fill="${o}" opacity="${e.monochrome?.5:.85}"/></svg>`};var Ye=`
